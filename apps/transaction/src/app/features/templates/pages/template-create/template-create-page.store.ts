...
export interface TemplateCreatePageState {
  data: TemplateCreateType | null;
  loading: boolean;
}

@Injectable()
export class TemplateCreatePageStore extends ComponentStore<TemplateCreatePageState> {
  ...
  private readonly updateData = this.updater(
    (state, input: TemplateCreateType) => ({
      ...state,
      data: input,
    })
  );
